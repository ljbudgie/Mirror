...
If yes, the decision can be treated as **SOVEREIGN** — but only when that human review is real, evidenced, and specific. If no, the decision is **NULL** under the Burgess Principle. If the institution answers with vague process language ("human oversight", "reviewed in line with policy") without confirming a named reviewer looked at the specific facts, the answer is **AMBIGUOUS** and should be treated as NULL until clarified.

NULL is not a final verdict — it is the documented starting point for escalation and repair. This operationalises the *meaningful human involvement* requirement in the **Data (Use and Access) Act 2025** (s.80) / **UK GDPR Articles 22A–22D** (in force 5 February 2026) and **EU AI Act Article 14**.

MIT licence. Python ≥ 3.11. Aligned with the Burgess Principle framework v2.6.6.
...
- **SOVEREIGN / NULL / AMBIGUOUS clarity:** vague process language is not enough; either the institution can evidence meaningful human review (SOVEREIGN) or it cannot (NULL) — and anything in between is AMBIGUOUS until clarified.
...
### The Burgess engine: SOVEREIGN / NULL / AMBIGUOUS

At the heart of Mirror is `core/burgess.py`, the deterministic classification engine that applies the Burgess Principle to an institution's response and returns one of three outcomes:
...
- **AMBIGUOUS** — vague process language without confirming a named reviewer looked at the specific facts. Treat as NULL until clarified.

For a response to count as SOVEREIGN, the institution must evidence all five criteria: a **named reviewer**, their **role**, the **specific facts** they reviewed, that it happened **before** the decision, and that they had **authority** to change the outcome.

The engine carries the legal foundations that give a NULL finding weight:

- **Data (Use and Access) Act 2025, s.80** (in force 5 February 2026) — defines an unlawful automated decision as one made without meaningful human involvement, and grants a right to representations, a named human reviewer with authority to change the outcome, and to contest the decision.
- **UK GDPR Articles 22A–22D** — the statutory meaningful-human-involvement requirement.
- **EU AI Act Article 14** — effective human oversight of high-risk AI systems.
- **HM Treasury v Ahmed (No. 2) [2010] UKSC 5** and **R (Majera) v SSHD [2021] UKSC 46** — an act taken without required individual human review may be *void ab initio*.

The same three-outcome logic is mirrored in the local web interface: paste an institution's reply starting with `check` and Mirror classifies it locally, in your browser.

The Python toolkit runs the same loop end to end. `advise_from_response(domain, reply)` classifies the institution's reply and hands back exactly one next step — SOVEREIGN moves you on to work through the substance of the response, while NULL and AMBIGUOUS route you to escalation. The Burgess engine and this bridge are exported from the `core` package:

```python
from core import advise_from_response

advice = advise_from_response("platform", "This was an automated decision generated by our system.")
print(advice.result.outcome)   # Outcome.NULL
print(advice.next_step.action) # the single next action to take
```
...
```text
/core        Python modules — conversation, rights, Burgess engine, next step, templates, commitment, timeline, AI adapter
...
- Article 22A–22D automated decision challenges (Data (Use and Access) Act 2025)
- ICO complaints
- NHS complaints
